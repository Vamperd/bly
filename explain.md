...
A与B评测的Mask不同，所以不能把两组数值当成严格的一对一竞赛；但两者都卡在State，足以支持一次约51M、30k步的H50-A规模复核。它只做一次：若更宽模型仍失败，就停止继续堆参数；只有它通过，才以H50继续固定和随机物理Mask。
...
F4G-O已经证明目标函数和评测门禁存在解析可达解；H38-A与H38-B则表明，37.57M层级模型在既定30k/60k预算内，无论只靠latent还是结合固定物理条件，都仍卡在State时序重建。当前只做一次51.01M的H50-A规模复核，不再延长H38或直接进入随机Mask。
