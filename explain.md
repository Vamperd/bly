...
现在新增的“精确初始化重放”就是给这把尺子重新校零。采集时机器人不仅关节位置不同，关节零点、质量、惯量、质心和摩擦也经过随机化；旧重放只创建了一个干净的新机器人。新流程从训练HDF取回这些参数和第一帧的完整姿态，在两个完全独立的Isaac进程中分别播放原Action和H50-A Action，同时把训练HDF直接做成第三份视频。三条都只看同一段64步。如果原Action仍回不到训练轨迹，最诚实的结论不是“模型差”，而是数据里还缺少接触缓存一类仿真器内部状态；只有原Action先通过，才有资格判断H50-A重放好不好。当前代码已经准备好，Ubuntu结果尚未产生。
