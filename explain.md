...
因此现在允许一次最多15k步的“小步精修”。它像在已经接近合格线的答案上用较细的笔继续修正：保留模型和优化器已经积累的方向，只把学习率温和升到原峰值的很小一部分，再慢慢降回去。若连续三次评测合格就提前结束；若15k后仍不合格，就不再继续堆参数，而是专门处理关节速度和最差State窗口。
...
| H50-A续训 | 保留现有解继续精修是否能稳定过线 | 尚未执行 | PASS进H50-B；FAIL停止扩模 |
| H38-R | 已见序列上的随机物理Mask补全 | 因B失败而BLOCKED | 只有同profile的B通过才运行 |
...
F4G-O已经证明目标函数和评测门禁存在解析可达解；H38-A与H38-B则表明，37.57M层级模型在既定30k/60k预算内，无论只靠latent还是结合固定物理条件，都仍卡在State时序重建。51.01M的H50-A已经显著改善平均误差但以3.71%的小差距失败，因此当前只做一次有上限的15k尾段续训，不延长H38、建立更大模型或直接进入随机Mask。

只有同一profile的fixed与random阶段最终通过，我们才有一个可信的KL=0重建基线。之后再加入概率分布和KL，并公平比较posterior mean、posterior sample与conditional prior sample，才能回答这个模型是否真正成为可用的条件CVAE。
