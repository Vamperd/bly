...
即使把门槛放宽到global `0.02`、worst-window `0.04`、p99 `0.06`并取消max-abs门禁，它仍不能通过；三项分别还高约24.7%、4.4%和25.4%。这说明它已经明显接近宽松目标，但不能把“接近”写成PASS。

H38-B随后从A的最佳权重继续训练60k步，改用8类物理上有意义的缺口。它也没有通过：global State/Action约为`0.02499/0.01641`，最坏窗口约为`0.04708/0.02507`，p99约为`0.08157`。脚接触和latent替换检查通过；宽松门槛下仍由p99控制失败。
...
A与B评测的Mask不同，所以不能把两组数值当成严格的一对一竞赛；但两者都卡在State，足以支持一次约51M的H50-A规模复核。它只做一次：若更宽模型仍失败，就停止继续堆参数；只有它通过，才以H50继续固定和随机物理Mask。
