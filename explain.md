...
H50-CPD曾尝试这一层的确定性版本：让Mask序列模仿H50-A的固定latent，再用冻结decoder补全。正式实验中latent模仿已经很准，但重建仍很差，说明旧decoder对略有偏差的latent过于敏感。随后H50-SCVAE让posterior、prior和decoder共同形成更适合条件补全的新表达；固定物理Mask训练跑满60k后仍未通过。prior和posterior的“便签”已经非常相似，但两者写出的State都不够准，说明当前不是prior单方面找不到posterior，而是联合训练没有先把共享解码能力打牢。它仍不能写成“模型已经学会State到Action推理”。
...
为了先直观看清H50-A已经学到了什么，当前增加了一次不训练的单窗口视频检查：从它真正见过的`variant 0 / T64`窗口取出完整序列，让posterior生成便签，在decoder端把State和Action全部遮住，再重建64步Action。随后把原Action和重建Action放进相同的Isaac环境各跑一次，和训练时记录的轨迹组成三栏视频。原Action重放是必要的“尺子”：如果重新启动仿真本身就与训练记录不同，就不能把这部分差异算到模型头上。这仍只展示H50-A的已见数据记忆，不是根据残余条件猜缺口。

这次视频检查已经完成。模型预测Action与真值平均只差`0.00544 rad`，约`0.31°`；在完全相同的重新仿真条件下，原Action与预测Action产生的关节姿态平均差约`0.00845 rad`，身体关键点平均差约`4.6 mm`，说明右栏基本跟随中栏。但原Action重新播放也没有回到左栏的训练轨迹：身体关键点平均相差约`0.419 m`，最大朝向差约`107°`。所以这段视频能说明模型把该窗口Action记得很准，却不能说明重新仿真恢复了当时采集到的动作；后一个问题首先是重放初态或环境上下文没有完整复现。

现在新增的“精确初始化重放”就是给这把尺子重新校零。采集时机器人不仅关节位置不同，关节零点、质量、惯量、质心和摩擦也经过随机化；旧重放只创建了一个干净的新机器人。新流程从训练HDF取回这些参数和第一帧的完整姿态，在两个完全独立的Isaac进程中分别播放原Action和H50-A Action，同时把训练HDF直接做成第三份视频。首次运行因Isaac地面材质路径差异在0 Action处停止；修复后正式运行的两次初始化一致，原Action先成功复现训练记录，H50-A Action也通过同一窗口的物理重放。这说明旧视频左/中不一致主要是初始化复现问题，并支持H50-A确实记住了这一个训练窗口的Action；仍不能说明模型会根据陌生的Mask条件生成，也不能外推到未见动作。

为了更容易看懂完整动作，后续视频不再只截取1.3秒。三条视频都从episode第一帧播放到最后一帧：原始回放始终使用记录Action；H50-A回放仅在选中的64步区间使用模型还原Action，前后仍使用原始Action。底部进度条把这段区间标红。因此视频会展示动作的完整上下文，同时红色部分准确说明“模型真正负责了哪里”；红色区间之外不能算作模型生成结果。

完整episode回放也暴露了一个重要限制：记录Action是开环播放，数秒后即使原Action也可能因微小物理误差累积而跌倒。为了继续查看模型本身的短程效果，工具保留了64步短视频模式；它直接从待测试窗口的真实首帧启动，避免把窗口之前已经积累的偏差算到模型头上。短视频仍只是已见窗口的posterior定性检查。
...
| H50-SCVAE | q、p与共享decoder能否共同学习标准条件生成 | 固定Mask训练60k；q-p对齐通过，但q/p重建都FAIL | 改为先q、再p、最后联合的分阶段均值课程 |
...
第一次让“两位记录员和剪辑师同时重新约定语言”的60k实验没有成功。两位记录员写出的便签已经几乎一致，说明prior并没有远远落后于posterior；但即使使用能看完整录像的posterior便签，State重建仍超出门槛。这更像三个人同时改写暗号和剪辑规则，始终没有先把一条可靠流程固定下来。下一轮因此不再原样加时，而是先让posterior与decoder把固定Mask重建学稳，再让prior追上，最后才短程联合调整。
