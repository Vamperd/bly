...
RMSE 0.0230198/0.0152627、max abs 0.215608；`quality_pass=false`是2-step smoke固定语义。当时的下一步
是从原F4D `best_progression.pt`独立执行C正式10k，不得从smoke checkpoint续训；该步骤现已完成。

F4C正式10k随后在run
`/home/helloworld/bly/runs/cvae_posterior_capacity_ab_c_seed20260830_20260907_004301`完整执行，源码
...
首次Ubuntu G8 smoke在optimizer构造阶段工程失败：固定配置使用`code_learning_rate`与
...
也已通过工程审计，源码同为`478f479bb3345697ff5c7da14583c9c3e13be193`。T129 code shape
`[80,129,16]`、165,120个code参数、每window 2,064标量，总参数25,625,059；4 motion/80 windows/
800 fixtures、双seed、encoder零调用/零梯度、训练身份及best/last checkpoint读回均正确。其两步
sample identity与G8逐步完全相同。回传中的F4E授权`null`来自查询了不存在的
...
也已完成固定15k且工程PASS、质量FAIL，源码`6e7caed535721a5ee575b80eba138cb6e392152e`。
最佳step15k的worst State/Action RMSE为0.117395/0.065137、max abs 2.909683、global State/Action
RMSE为0.040533/0.025330、contact 100%、53.122%元素超1e-2，zero/cross-window/cross-motion依赖
为18.94/16.58/20.97。13k/14k/15k score为306.470/295.692/290.968，三点均FAIL；全部10类Mask
的max abs均高于1e-2。15k训练身份、best/last读回、F4E授权及五张图均完整；step1与15k身份hash
...
H38-B正式run
`/home/helloworld/bly/runs/cvae_posterior_hierarchical_t64_h38_fixed_20260908_133755`已从H38-A
`best_fit.pt`做model-only初始化并跑满60k，源码`c5932690f43b478fb05687ccf58e6834b0243a32`。
