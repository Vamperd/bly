...
posterior_hierarchical_t64() {
  local stage="$1" smoke="$2" continuation="${3:-false}" dataset_run="${CVAE_DATASET_RUN:-}"
  ...
  local resume_run="${CVAE_POSTERIOR_HIERARCHICAL_RESUME_RUN:-}"
  ...
  if [[ "$continuation" == "true" ]]; then
    [[ "$profile" == "H50" && "$stage" == "autoencode" && "$smoke" == "false" ]] \
      || die "hierarchical continuation is restricted to formal H50 autoencode"
    [[ -z "$init_checkpoint" ]] \
      || die "continuation restores last.pt directly; unset CVAE_POSTERIOR_HIERARCHICAL_INIT_CHECKPOINT"
    [[ -z "$h38_failed_run" ]] \
      || die "continuation revalidates the H50 source; unset CVAE_POSTERIOR_H38_FAILED_RUN"
    [[ -n "$resume_run" ]] \
      || die "CVAE_POSTERIOR_HIERARCHICAL_RESUME_RUN is required for H50 continuation"
    [[ -f "$resume_run/manifests/posterior_hierarchical_t64_summary.json" ]] \
      || die "H50 continuation source summary is missing: $resume_run"
    [[ -f "$resume_run/checkpoints/last.pt" ]] \
      || die "H50 continuation source last.pt is missing: $resume_run"
    [[ -f "$resume_run/markers/cvae_posterior_hierarchical_t64_execution.ok" ]] \
      || die "H50 continuation source execution marker is missing: $resume_run"
    [[ -f "$resume_run/markers/cvae.failed" ]] \
      || die "H50 continuation requires the narrowly failed formal source: $resume_run"
  elif [[ "$stage" == "autoencode" ]]; then
    [[ -z "$resume_run" ]] \
      || die "CVAE_POSTERIOR_HIERARCHICAL_RESUME_RUN is accepted only by the continuation entry"
    [[ -z "$init_checkpoint" ]] \
  ...
  [[ "$continuation" == "true" ]] && prefix="${prefix}_continue15k"
  [[ "$smoke" == "true" ]] && prefix="${prefix}_smoke"
  run_dir="$(new_run_dir "$prefix")"
  capture_environment "$run_dir"
  local extra_args=()
  ...
  [[ -n "$resume_run" ]] && extra_args+=(--resume-run "$resume_run")
  [[ "$smoke" == "true" ]] && extra_args+=(--smoke)
  ...
  [[ "$continuation" == "true" ]] && marker="cvae_posterior_hierarchical_t64_continuation_execution.ok"
  ...
  [[ "$continuation" == "true" ]] && latest_key="${latest_key}_continue15k"
  [[ "$smoke" == "true" ]] && latest_key="${latest_key}_smoke"
  update_latest "$latest_key" "$run_dir"
  printf '%s\n' "$run_dir"
}

posterior_hierarchical_conditional_prior() {
  local mode="$1" smoke="$2" dataset_run="${CVAE_DATASET_RUN:-}"
  local source_run="${CVAE_POSTERIOR_PRIOR_SOURCE_RUN:-}"
  local init_run="${CVAE_POSTERIOR_PRIOR_INIT_RUN:-}"
  local config="$SCRIPT_DIR/configs/posterior_hierarchical_conditional_prior_h50.json"
  local prefix run_dir marker latest_key
  [[ -z "${CVAE_CONFIG:-}" ]] \
    || die "posterior-hierarchical-prior uses its locked config; unset CVAE_CONFIG"
  [[ -n "$dataset_run" ]] || die "CVAE_DATASET_RUN is required"
  [[ -f "$dataset_run/markers/cvae_overfit_subset.ok" ]] \
    || die "dedicated overfit subset marker is missing: $dataset_run"
  [[ -n "$source_run" ]] || die "CVAE_POSTERIOR_PRIOR_SOURCE_RUN is required"
  ...
  local extra_args=()
  if [[ "$mode" == "adapt" ]]; then
    [[ "$smoke" == "false" ]] || die "decoder adaptation has no smoke shortcut"
    [[ -n "$init_run" ]] || die "CVAE_POSTERIOR_PRIOR_INIT_RUN is required for decoder adaptation"
    [[ -f "$init_run/manifests/posterior_conditional_prior_summary.json" ]] \
      || die "conditional-prior training summary is missing: $init_run"
    [[ -f "$init_run/markers/cvae_posterior_conditional_prior_latent_alignment.ok" ]] \
      || die "decoder adaptation requires the latent-alignment marker"
    [[ -f "$init_run/markers/cvae.failed" ]] \
      || die "decoder adaptation is only allowed after reconstruction quality failure"
    extra_args+=(--init-run "$init_run")
  else
    [[ "$mode" == "train" ]] || die "invalid conditional-prior mode: $mode"
    [[ -z "$init_run" ]] || die "fresh prior training requires CVAE_POSTERIOR_PRIOR_INIT_RUN to be unset"
  fi
  [[ "$smoke" == "true" ]] && extra_args+=(--smoke)
  prefix="cvae_posterior_hierarchical_prior_h50_cpd_${mode}"
  [[ "$smoke" == "true" ]] && prefix="${prefix}_smoke"
  run_dir="$(new_run_dir "$prefix")"
  capture_environment "$run_dir"
  run_logged "$run_dir" posterior_hierarchical_conditional_prior.log \
    "$PYTHON" -m cvae_sa.posterior_hierarchical_conditional_prior \
      --dataset-run "$dataset_run" \
      ...
      --mode "$mode" \
      "${extra_args[@]}"
  marker="cvae_posterior_conditional_prior_execution.ok"
  [[ "$smoke" == "true" ]] && marker="cvae_posterior_conditional_prior_smoke.ok"
  [[ -f "$run_dir/markers/$marker" ]] \
    || die "conditional-prior marker is missing: $marker"
  latest_key="posterior_hierarchical_prior_h50_cpd_${mode}"
  [[ "$smoke" == "true" ]] && latest_key="${latest_key}_smoke"
  update_latest "$latest_key" "$run_dir"
  printf '%s\n' "$run_dir"
}
...
posterior_h50_action_replay() {
  local dataset_run="${CVAE_DATASET_RUN:-}"
  local source_run="${CVAE_POSTERIOR_H50_REPLAY_SOURCE_RUN:-}"
  local checkpoint motion_key variant window_start post_steps replay_seed
  ...
  post_steps="${CVAE_POSTERIOR_H50_REPLAY_POST_STEPS:-50}"
  ...
  [[ -f "$checkpoint" ]] || die "H50-A replay checkpoint is missing: $checkpoint"
  [[ "$variant" =~ ^[0-9]+$ ]] || die "CVAE_POSTERIOR_H50_REPLAY_VARIANT must be non-negative"
  [[ "$window_start" =~ ^[0-9]+$ ]] \
    || die "CVAE_POSTERIOR_H50_REPLAY_WINDOW_START must be non-negative"
  [[ "$post_steps" =~ ^[0-9]+$ ]] \
    || die "CVAE_POSTERIOR_H50_REPLAY_POST_STEPS must be non-negative"
  [[ "$replay_seed" =~ ^[0-9]+$ ]] \
  ...
  run_dir="$(new_run_dir cvae_posterior_h50a_seen_window_action_replay)"
  capture_environment "$run_dir"
  run_logged "$run_dir" h50a_action_replay_prepare.log \
    "$PYTHON" -m cvae_sa.posterior_h50_action_replay prepare \
      --dataset-run "$dataset_run" \
      --source-run "$source_run" \
      --checkpoint "$checkpoint" \
      --output-run "$run_dir" \
      ...
      --seed "$replay_seed"
  run_logged "$run_dir" h50a_action_replay_isaac.log \
  ...
  run_logged "$run_dir" h50a_action_replay_render.log \
    "$PYTHON" "$SONIC_KIT_DIR/render_h50a_seen_window_action_replay.py" \
      --run-dir "$run_dir" \
      --model "$model_path" \
      --width "${CVAE_POSTERIOR_H50_REPLAY_WIDTH:-1440}" \
  ...
  [[ -s "$run_dir/videos/h50a_seen_window_action_replay.mp4" ]] \
    || die "H50-A replay renderer did not create the primary MP4"
  printf 'PASS\n' > "$run_dir/markers/h50a_action_replay_render.ok"
  run_logged "$run_dir" h50a_action_replay_finalize.log \
    "$PYTHON" -m cvae_sa.posterior_h50_action_replay finalize \
      --output-run "$run_dir"
  [[ -f "$run_dir/markers/cvae_posterior_h50_action_replay.ok" ]] \
    || die "H50-A seen-window Action replay marker is missing"
  update_latest posterior_h50_action_replay "$run_dir"
  printf '%s\n' "$run_dir"
}
...
case "${1:-}" in
  ...
  posterior-hierarchical-t64-continue) posterior_hierarchical_t64 autoencode false true ;;
  ...
  posterior-hierarchical-prior-smoke) posterior_hierarchical_conditional_prior train true ;;
  posterior-hierarchical-prior-train) posterior_hierarchical_conditional_prior train false ;;
