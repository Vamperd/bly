...
训练运行时可通过 `CVAE_POSTERIOR_MAX_STEPS`、`CVAE_POSTERIOR_MICRO_BATCH`、`CVAE_POSTERIOR_LEARNING_RATE`、`CVAE_POSTERIOR_LR_SCHEDULE`（`constant/cosine/linear`）、`CVAE_POSTERIOR_WARMUP_STEPS` 和 `CVAE_POSTERIOR_MIN_LR_RATIO` 覆盖 batch、步数与学习率曲线；Python 入口对应 `--micro-batch`、`--max-steps`、`--learning-rate`、`--lr-schedule`、`--warmup-steps`、`--min-lr-ratio`。这些只改训练协议，不改变架构签名。
...
每次完整评测还记录连续误差的全局 `continuous_abs_p95/p99`、State/Action 分域 p95/p99、按窗口的 `worst_window` 与 `worst_window_by_max_abs`，以及按连续特征排序的 `worst_state_features`、`worst_action_features`。这些连续误差均在数据归一化空间统计：`max_abs` 是所有有效连续 State/Action 目标元素中单个绝对误差的最大值（不含 contact BCE，也不含无效 terminal Action），属于尾部指标；它不能由平均 RMSE 推断，也不能单独作为总体 loss。

评测路由必须与训练阶段一致：Stage A 使用完整序列 Posterior mean；Stage B 使用固定 `training_mask_seed` 生成的 masked State/Action 条件并经 Condition Encoder；Stage C 使用同一条件路径和 posterior reparameterized sample。Stage B/C 不得复用 Stage-A 的无 Mask 评测结果，否则会把 posterior 重建误报为 condition 能力。

当前 B 阶段采用独立 condition-only 路由：`masked State–Action → Condition Encoder → condition memory/global/local → condition-only fusion + FiLM → Decoder → 完整 State/Action`。B 的 forward 不执行 Posterior Encoder，不生成 posterior latent，不计算 KL；`posterior` 在输出中为 `None`，`latent_source=condition_mean`。B 可以从 A checkpoint 取得共享 Decoder/fusion 的初始化，但若要检验 B 链路本身，推荐不设置 `CVAE_POSTERIOR_STANDARD_CVAE_INIT_RUN`，从随机初始化开始；此时 Posterior 参数保持冻结且完全不参与 forward，Condition Encoder、condition-only fusion、memory projection、FiLM、Decoder 和输出头训练。B 的完整评测同样使用固定条件 Mask，记录 `evaluation_stage=B` 和 masked-condition scope。

B evaluation 另外记录 `condition_mask_seed`、每个窗口的 `mask_name` 和 `mask_breakdown`，可分别查看 `state_gap_4`、`state_rollout`、`action_gap_16`、`full_action`、`joint_gap_8` 等 Mask 家族的平均/最差 State RMSE、Action RMSE、combined RMSE 和 max absolute error。
